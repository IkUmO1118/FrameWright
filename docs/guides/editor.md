...
エディタで収録を開いたとき、`transcript.json` が bootstrap の空文書のままなら
バックグラウンドで `transcribe` → `detect` が自動で走る。`plan` は走らないので、
cutplan / chapters / meta は書き換えられない。

自動解析でできた文字起こしは、最初は「未採用」の知覚データとして保存される。
この状態では AI 編集やスクリプトタブは発話内容を読めるが、プレビューやレンダーには
テロップとして出ない。字幕を載せたいときは、左パネルの「テロップ」タブに出る
**「文字起こしをテロップにする」**を押す。これは LLM を使わず、未採用マーカーを外す
だけなので、通常の編集と同じく ⌘Z で戻せて、⌘S で確定する。

- 文字起こしをテロップに入れる前に AI へテロップ編集を頼むと、先に
  「文字起こしをテロップにする」を押すよう案内される
- 解析中も再生・手動編集・保存はできる。AI 編集の入口だけ、文字起こし完了まで無効になる
- 解析中にテロップを保存した場合、文字起こし結果は破棄され、手編集が残る
- whisper モデルが未設定なら自動解析は走らない。理由は AI 編集ボタンの tooltip に出る
- 失敗した場合はトーストの「再試行」から同じ解析を再実行できる

カットの手編集を保ったまま章立て・タイトル・概要欄だけ作り直したいときは、
ターミナルで `node src/cli.ts remeta <dir>`。CLI で文字起こし・無音検出・plan まで
一気に作りたいときは `node src/cli.ts run <dir>`。
