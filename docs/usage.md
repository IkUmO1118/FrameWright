...
③ 必要なら明示実行:
     node src/cli.ts run <フォルダ>          CLI で AI 初版まで作る(transcribe→detect→plan)
     node src/cli.ts transcribe <フォルダ>   文字起こし
     node src/cli.ts plan <フォルダ>         AI カット案・章立て
     ※ GUI では、開いたときに必要なら文字起こしと無音検出だけ自動で走る(plan は走らない)
